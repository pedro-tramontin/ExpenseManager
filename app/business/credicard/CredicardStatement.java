...
public class CredicardStatement extends BaseStatementProcessor {
	...
	@Inject
	public CredicardStatement(EarnDao earnDao, ExpenseDao expenseDao, PeriodDao periodDao, CategoryDao categoryDao) {
		super();
	...
	public Iterator<CredicardStatementEntry> statementIterator() {
		if (!excel.isOpen()) {
			excel.openWorkbook();
		}

		return new CredicardStatementEntryIterator(excel);
	}
	...
	public ProcessorResponse run() throws ParseException {
		...
		listAutoFill.add(new AutoFill(".*NETFLIX.*", "Netflix", categoryDao.findCategory("Fixo p/ mês")));
		listAutoFill.add(new AutoFill(".*PLAYSTATION.*", "Playstation Store", categoryDao.findCategory("Diversos")));

		for (Iterator<CredicardStatementEntry> it = statementIterator(); it.hasNext();) {
			CredicardStatementEntry entry = it.next();
